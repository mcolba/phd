...
PARAM_COLS = ["kappa", "v_inf", "vol_of_vol", "rho", "v0"]
...
def _make_params(row: pd.Series) -> HestonParams:
    return HestonParams(*(float(row[col]) for col in PARAM_COLS))


def test_heston_price_cui_against_quantlib(ql_data: pd.DataFrame) -> None:
    sample = ql_data
    params = _make_params(sample.iloc[0])
    market = _make_market(sample.iloc[0])
    ...
    assert float(np.max(abs_error)) <= 1e-5
    assert float(np.max(rel_error[~idx_small_value])) <= 1e-3


def test_heston_delta_matches_finite_difference() -> None:
    params = HestonParams(
        kappa=1.5,
        v_inf=0.04,
        vol_of_vol=0.5,
    ...
    assert dll_delta == pytest.approx(fd_delta, abs=0.5 * 10**-4)


def test_heston_jacobian_against_finite_difference() -> None:
    params = HestonParams(
        kappa=1.5,
        v_inf=0.04,
        vol_of_vol=0.5,
...
def test_heston_calibrator_recovers_parameters() -> None:
    ...
    assert stats["error_l2_final"] < stats["error_l2_initial"]
    assert stats["stop_code"] in {1, 2, 6}
    assert_allclose(
        [
            par_opt.kappa,
            par_opt.v_inf,
