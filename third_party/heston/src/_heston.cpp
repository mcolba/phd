...
struct mktpara {
	...
	Eigen::ArrayXd T;
	Eigen::ArrayXd K;
	Eigen::ArrayXd r_termS; // interest rate term structure
	Eigen::ArrayXd S_adj;   // Adjusted Spot = S_0^{-q_i*dt_i} 

	// constructor: 
	mktpara() {}; // default 
	mktpara(int n); // parametrized 
};
mktpara::mktpara(int n) {
	(this->T).resize(n);
	(this->K).resize(n);
	(this->r_termS).resize(n);
	(this->S_adj).resize(n);
}
...
void dHes(double *p, double *delta, int m, int n, void *data, double *q_termS)
...
{
	// m = # of parameters
	// n = # of observations (consistent with the struct mktpara)

	struct mktpara market(*n);
	market.K = Eigen::Map<Eigen::ArrayXd>(K, *n);
	market.T = Eigen::Map<Eigen::ArrayXd>(mat, *n);
	market.r_termS = Eigen::Map<Eigen::ArrayXd>(r, *n);
	market.S_adj = Eigen::Map<Eigen::ArrayXd>(S_adj, *n);

	fHes(par, p, *m, *n, (void *)&market);
}
	...
	double *mat, double *par, int *n, int *m, double *delta)
{
	...
	dHes(par, delta, *m, *n, (void *)&market, q);
}
	...
	double *mat, double *par, int *n, int *m, double *jac)
{
	...
	JacHes(par, jac, *m, *n, (void *)&market);
}

// Heston Calibrator BIS (rates term structure)
extern "C" __declspec(dllexport) void hestonCalibrator(double *S_adj, double *r, double *K,
	double *mat, double *price, double *par, double *statistics, int *n, int *m, bool *print)
{
	struct mktpara market(*n);
	market.K = Eigen::Map<Eigen::ArrayXd>(K, *n);
	market.T = Eigen::Map<Eigen::ArrayXd>(mat, *n);
	...
	// par[0]                        | kappa                         |  mean reversion rate
	// par[1]                        | v_infinity                    |  long term variance
	// par[2]                        | sigma                         |  variance of volatility
	// par[3]                        | rho                           |  correlation between spot and volatility
	// par[4]                        | v0                            |  initial variance
	...
	opts[1] = 1E-10;       // ||J^T e||_inf	 | The magnitude of the gradient of ||e||_2 -> it zero at the min point 
	opts[2] = 1E-10;       // ||Dp||_2		 | The relative change in the magnitude of delta_par 
	opts[3] = 1E-10;       // ||e||_2		 | The error 
	opts[4] = LM_DIFF_DELTA; // finite difference if used

	// Calibrate using analytical gradient
	dlevmar_der(fHes, JacHes, par, price, *m, *n, 1500, opts, info, NULL, NULL, (void *)&market);
	
	//CONSTRAINED OPTIMIZATION
	// double lb[5] = { 0.005,0.005,0.005,-1,0.005 };
	// double ub[5] = { 8,5,5,+1,5 };
	// dlevmar_bc_der(fHes, JacHes, par, price, *m, *n, lb, ub, NULL, 200, opts, info, NULL, NULL, (void *)&market);
	
	
	/* dlevmar_der ARGUMENTS: 
		* pointers to routines evaluating the vector function f and its Jacobian (if applicable), 
		* pointers to the initial estimate of the parameter vector p and the measurement vector x, 
		* the dimension m of p, 
		* the dimension n of x, 
		* the maximum number of iterations, 
		* 4 element array containing respectively:
			* scale factor for initial \mu, 
			* stopping thresholds for ||J^T e||_inf, ||Dp||_2 and ||e||_2
		* a pointer to an array into the elements of which various pieces of information regarding the result of minimization will be returned, 
		* a pointer to working memory, 
		* a pointer to a covariance matrix corresponding to LS solution, and 
		* a pointer to application-specific data structures. 
	for more details, http://users.ics.forth.gr/~lourakis/levmar/, http://users.ics.forth.gr/~lourakis/levmar/levmar.pdf, 
	or http://www2.imm.dtu.dk/pubdb/views/edoc_download.php/3215/pdf/imm3215.pdf

	for BLEIC source code see e.g. 
	https://github.com/tszalay/viewpad/blob/master/levmar-2.5/lmbleic_core.c
	*/
	...
	if (*print==TRUE){
		...
		cout << " Optimum found:" << scientific << setprecision(8) << "\t" << par[0] << "\t" << par[1] << "\t" << par[2] << "\t" << par[3] << "\t" << par[4] << endl;
	...
	statistics[4] = double(stop_s - start_s)/CLOCKS_PER_SEC; 
}